...
export default function ClusteringPagev2() {
  ...
  async function runClustering() {
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:5000/api/setup/run_clustering",
        {
          method: "POST",
          body: params,
        }
      ...
      if (response.ok) {
        ...
        const radiiData = Object.values(responseData).map(
          (cluster) => cluster[1]
        );
  ...
  async function confirmClustering() {
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:5000/api/setup/confirm_clustering",
        {
          method: "POST",
          body: params,
        }
  ...
  async function assignForSearch() {
    try {
      const response = await fetch(
        "http://127.0.0.1:5000/api/setup/start_operation",
        {
          method: "GET",
        }
      );
  ...
  const addHotspot = (latlng) => {
    ...
    setHotspots((prevHotspots) => [
      ...prevHotspots,
      { position: latlng, icon: createHotSpotIcon() },
    ]);
  };
  ...
  return (
    <>
            ...
            {hotspots.map((hotspot, index) => (
              <React.Fragment key={index}>
                <Box
                  ...
                  <Typography>
                    Latitude: {hotspot.position.lat.toFixed(3)} Longitude:{" "}
                    {hotspot.position.lng.toFixed(3)}
                  </Typography>
                  <IconButton
                    aria-label="delete"
          ...
          {clusters.map((cluster, index) => (
            <React.Fragment key={index}>
              <Box
                ...
                <Typography>
                  Lat: {cluster.position.lat.toFixed(3)} Lon:{" "}
                  {cluster.position.lng.toFixed(3)}
                </Typography>
                {clusterRadii[index] ? (
                  <Typography>
      ...
      <MapContainer
        ...
        {hotspots.map((hotspot, index) => (
          <Marker key={index} position={hotspot.position} icon={hotspot.icon}>
            <Popup>
